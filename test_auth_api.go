...
func main() {
	...
	token, user := registerTestUser()
	log.Printf("Registered user: %s with ID: %d\n", user.Username, user.ID)

	// Test user login
	log.Println("Testing POST /users/login")
	loginToken, loginUser := loginTestUser(user.Username)
	log.Printf("Logged in user: %s with ID: %d\n", loginUser.Username, loginUser.ID)

	// Verify login token works too
	log.Println("Verifying login token works")
	loginCurrentUser := getCurrentTestUser(loginToken)
	log.Printf("Current user from login token: %s\n", loginCurrentUser.Username)

	// Test getting current user
	log.Println("Testing GET /users/me")
	currentUser := getCurrentTestUser(token)
...
func registerTestUser() (string, models.User) {
	...
	resp, err := http.Post(authBaseURL+"/users/register", "application/json", bytes.NewBuffer(jsonData))
	if err != nil {
...
func loginTestUser(username string) (string, models.User) {
	...
	resp, err := http.Post(authBaseURL+"/users/login", "application/json", bytes.NewBuffer(jsonData))
	if err != nil {
...
func getCurrentTestUser(token string) models.User {
	req, err := http.NewRequest(http.MethodGet, authBaseURL+"/users/me", nil)
	if err != nil {
...
func createTestPost(token string) models.Post {
	...
	req, err := http.NewRequest(http.MethodPost, authBaseURL+"/posts", bytes.NewBuffer(jsonData))
	if err != nil {
		log.Fatalf("Failed to create request: %v", err)
	}
...
func getAllTestPosts(token string) []models.Post {
	req, err := http.NewRequest(http.MethodGet, authBaseURL+"/posts", nil)
	if err != nil {
		log.Fatalf("Failed to create request: %v", err)
	}
	req.Header.Set("Authorization", "Bearer "+token)
...
func getTestPost(token string, id int) models.Post {
	req, err := http.NewRequest(http.MethodGet, fmt.Sprintf("%s/posts/%d", authBaseURL, id), nil)
	if err != nil {
		log.Fatalf("Failed to create request: %v", err)
	}
	req.Header.Set("Authorization", "Bearer "+token)
...
func updateTestPost(token string, id int) models.Post {
	...
	req, err := http.NewRequest(http.MethodPut, fmt.Sprintf("%s/posts/%d", authBaseURL, id), bytes.NewBuffer(jsonData))
	if err != nil {
		log.Fatalf("Failed to create request: %v", err)
	}
	req.Header.Set("Content-Type", "application/json")
...
func deleteTestPost(token string, id int) {
	req, err := http.NewRequest(http.MethodDelete, fmt.Sprintf("%s/posts/%d", authBaseURL, id), nil)
	if err != nil {
		log.Fatalf("Failed to create request: %v", err)
	}
	req.Header.Set("Authorization", "Bearer "+token)
